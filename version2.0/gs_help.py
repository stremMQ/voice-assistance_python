...
def safe_settings():
    ...
    with sq.connect("settings") as con:
        cur = con.execute('''UPDATE setting SET golos = (?)''',(golos_i,))
    settings()
...
label_golos = ctk.CTkLabel(frame2, text = f"Выбран микровон: ", wraplength = 400)
combobox_golos = ctk.CTkComboBox(frame2, values = ["aidar", "baya", "kseniya", "xenia", "eugene"], width= 285 )
...
def commands():
    ...
    if command == []:
        ...
        label_golos.pack_forget()
        combobox_mic.pack_forget()
        combobox_golos.pack_forget()
        button_delite.pack_forget()
        button_create.pack_forget()
        label_spis_commands.pack_forget()
...
def list_golos(igs):
    global label_golos
    label_golos = ctk.CTkLabel(frame2, text=f"Выбран голос: {igs}", wraplength=400)
...
def settings():
    ...
    combobox_mic.pack(anchor = "center", pady = (10,0))

    label_golos.pack(anchor="center", pady=(20, 0))

    combobox_golos.pack(anchor = "center", pady = (10,0))

    button_safe.pack(pady = (10,0))
...
label_hello = ctk.CTkLabel(master=frame2,text = f"Голосовой помощник Lumen приветствует вас!", fg_color="#171717", corner_radius= 5)
